// RunMeasure V7.5 — QR order and safety
(() => {
    ...
    switch(state.mode){
      case 'intervalles':
        ...
        params.innerHTML = `
          ...
          <div class="info">Appuie sur « Tour ». À la fin : QR auto (par défaut <em>cumulés</em>), bascule possible <em>par intervalle</em>.</div>
        `; state.targetDist=800; state.splitDist=200; break;
      case 'simple':
        modeName.textContent='Chrono'; params.innerHTML = `<div class="info">Chrono simple. Pas de tours. <strong>Pas de QR.</strong></div>`; btnLap.classList.add('hidden'); btnStop.classList.remove('hidden'); tableWrap.classList.add('hidden'); break;
      case 'simpleDistance':
      ...
      case 'tours':
        modeName.textContent='Minuteur avec distance'; params.appendChild(buildMinuteSecondSelectors()); params.insertAdjacentHTML('beforeend', `<label>Distance par tour (m)<input type="number" id="p-lapdist" min="25" step="25" value="100"/></label><div class="info">Décompte + « Tour ». Tableau : <em>Temps cumulé</em>, <em>Temps tour</em>, <em>Vitesse</em>, <em>Distance cumulée</em>.</div>`); state.countdownMs=mmssToMsStr(5,0); state.ringTotal=state.countdownMs; state.lapDist=100; circleWrap.classList.remove('hidden'); display.classList.add('hidden'); liveDistance.classList.remove('hidden'); updateRing(state.countdownMs,state.ringTotal); break;
      case 'demiCooper':
      case 'cooper':
        modeName.textContent = (state.mode==='demiCooper')?'Demi-Cooper (6′)':'Cooper (12′)'; const fixed = (state.mode==='demiCooper')? 6*60*1000 : 12*60*1000;
        params.innerHTML = `<label>Distance par tour (m)<input type="number" id="p-lapdist" min="25" step="25" value="100"/></label><div class="info">Durée fixe ${(fixed/60000)}:00. Tableau identique. Fractions mises à jour partout.</div>`; state.fixedDuration=fixed; state.ringTotal=fixed; state.lapDist=100; circleWrap.classList.remove('hidden'); display.classList.add('hidden'); liveDistance.classList.remove('hidden'); updateRing(state.fixedDuration, state.ringTotal); break;
      case 'minuteurSimple':
        modeName.textContent='Minuteur'; params.appendChild(buildMinuteSecondSelectors()); params.insertAdjacentHTML('beforeend', `<div class="info">Compte à rebours simple. <strong>Pas de QR.</strong></div>`); state.countdownMs=mmssToMsStr(5,0); state.ringTotal=state.countdownMs; circleWrap.classList.remove('hidden'); display.classList.add('hidden'); btnLap.classList.add('hidden'); btnStop.classList.remove('hidden'); tableWrap.classList.add('hidden'); updateRing(state.countdownMs, state.ringTotal); break;
    }
  ...
  function generateQR(){ const p=buildPayload(); qrcodeBox.innerHTML=''; if(!p) return; try{ new QRCode(qrcodeBox,{ text:JSON.stringify(p), width:200, height:200, correctLevel:QRCode.CorrectLevel.M }); }catch(e){ const d=document.createElement('div'); d.textContent='Erreur QR: '+e.message; qrcodeBox.appendChild(d);} }
  function buildPayload(){ if(state.mode==='intervalles'){ const type=(document.querySelector('input[name=\"ivl-type\"]:checked')||{}).value || 'cumules'; if(type==='cumules'){ const core={}; state.laps.forEach((l,i)=>{ const d=(i+1)*state.splitDist; core[`t${d}`]=fmt(l.cumMs); }); return payload('Temps intermédiaire', core);} else { const core={}; state.laps.forEach((l,i)=>{ core[`i${i+1}`]=fmt(l.lapMs); }); return payload('Temps intermédiaire', core);} } if(state.mode==='simpleDistance'){ const d=Math.round(state.targetDist||0); const core={}; core[`temps_${d}m`]=fmt(state.elapsed); core[`vitesse_${d}m`]=kmh(d,state.elapsed); return payload('Chrono avec calcul de vitesse', core);} if(state.mode==='tours'){ const dist=Math.round(state.cumDist + state.fractionAdded); return payload('Minuteur avec distance', { distance_totale:dist, vitesse_moyenne: kmh(dist,state.elapsed) }); } if(state.mode==='demiCooper'){ const dist=Math.round(state.cumDist + state.fractionAdded); const v=kmh(dist,state.elapsed); return payload('Demi-Cooper (6′)', { vma:v, distance:dist }); } if(state.mode==='cooper'){ const dist=Math.round(state.cumDist + state.fractionAdded); const v=kmh(dist,state.elapsed); return payload('Cooper (12′)', { vma:v, distance:dist }); } return null; }
  function csvExport(){ const s=student(); const esc=(v)=>(`\"${String(v).replace(/\"/g,'\"\"')}\"`); const lines=[]; lines.push(['nom','prenom','classe','sexe','lap_index','t_cum','t_lap','v_lap_kmh','dist_cum_m']); state.laps.forEach(l=>{ const v=l.lapDist? kmh(l.lapDist,l.lapMs).toFixed(2):''; lines.push([s.nom||'',s.prenom||'',s.classe||'',s.sexe||'', l.idx, fmt(l.cumMs), fmt(l.lapMs), v, (l.cumMeters||'')]); }); const totalMeters=(['tours','demiCooper','cooper'].includes(state.mode))? Math.round(state.cumDist + state.fractionAdded) : (state.mode==='intervalles'? Math.round(state.targetDist): ''); lines.push(['TOTAL','','','', '', fmt(state.elapsed), '', ( totalMeters? kmh(totalMeters,state.elapsed).toFixed(2):''), totalMeters]); const csv = lines.map(r=>r.map(esc).join(',')).join('\\r\\n'); const blob=new Blob([csv],{type:'text/csv;charset=utf-8;'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=`runmeasure_${(s.nom||'')}_${(s.prenom||'')}.csv`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url); }
  btnStart.addEventListener('click', start); btnStop.addEventListener('click', stop); btnLap.addEventListener('click', lap);
  btnReset.addEventListener('click', ()=>{ state.running=false; cancelAnimationFrame(state.raf); state.startTime=0; state.elapsed=0; state.lastLapAt=0; state.laps=[]; state.cumDist=0; state.fractionAdded=0; display.textContent='00:00.0'; if(circleWrap && !circleWrap.classList.contains('hidden')){ if(state.ringTotal) updateRing(state.ringTotal, state.ringTotal);} btnStart.textContent='Démarrer'; btnLap.disabled=(['minuteurSimple','simple','simpleDistance'].includes(state.mode)); btnStop.disabled=true; btnReset.disabled=false; if(!['simple','simpleDistance','minuteurSimple'].includes(state.mode)) tableWrap.classList.add('hidden'); results.classList.add('hidden'); ivlExportBox.classList.add('hidden'); fractionTools.classList.add('hidden'); qrcodeBox.innerHTML=''; liveDistVal.textContent='0'; recap.classList.add('hidden'); recapBody.innerHTML=''; if(totalDistanceCell) totalDistanceCell.textContent='—'; clearTable(); });
  document.getElementById('btn-new').addEventListener('click', ()=> location.reload()); document.getElementById('btn-export-csv').addEventListener('click', csvExport);
  renderParams();
})();