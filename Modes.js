...
(() => {
  ...
  const fmt = (ms) => { const t=Math.round(ms/100);const m=Math.floor(t/600);const s=Math.floor((t%600)/10);const d=t%10;return `${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}.${d}`; };
  ...
  const modeName=$('#mode-name'), params=$('#params'), display=$('#display'), btnStart=$('#btn-start'), btnLap=$('#btn-lap'), btnStop=$('#btn-stop'), btnReset=$('#btn-reset');
  const tableWrap=$('#table-wrap'), tbody=$('#laps-table tbody'), results=$('#results'), qrcodeBox=$('#qrcode'), totalTime=$('#total-time'), totalSpeed=$('#total-speed'), totalDistanceCell=$('#total-distance'), rowTotal=$('#row-total');
  const circleWrap=$('#circle-wrap'), liveDistance=$('#live-distance'), liveDistVal=$('#live-dist-val'), fractionTools=$('#fraction-tools'), recap=$('#recap'), recapBody=$('#recap-body');
  ...
  function renderParams(){
    ...
    switch(state.mode){
      ...
      case 'tours':
        ...
        circleWrap.classList.remove('hidden'); circleWrap.classList.add('huge'); display.classList.add('hidden'); liveDistance.classList.remove('hidden'); updateRing(state.countdownMs, state.ringTotal);
        break;
  ...
  function clearTable(){ tbody.innerHTML=''; rowTotal.classList.add('hidden'); totalTime.textContent='—'; totalSpeed.textContent='—'; totalDistanceCell.textContent='—'; }
