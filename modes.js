// RunMeasure — modes.js (QR plats, sans millisecondes, pour ScanProf)
(() => {
  const $ = (s, root=document) => root.querySelector(s);

  // Formats
  const fmt = (ms) => { // MM:SS.d (pour affichage live)
  ...
  const modeName=$('#mode-name'), params=$('#params'), display=$('#display'),
        ...
        results=$('#results'), qrcodeBox=$('#qrcode'), totalTime=$('#total-time'), totalSpeed=$('#total-speed'),
  ...
  function renderParams(){
    ...
    switch(state.mode){
      ...
      case 'simple':
        ...
        params.innerHTML = `<div class="info">Chrono simple. Pas de tours. <strong>Pas de QR.</strong></div>`;
      ...
      case 'simpleDistance':
        ...
        params.innerHTML=`<label>Distance (m)<input type="number" id="p-target" min="25" step="25" value="100"/></label><div class="info">Lance puis « Stop » à l'arrivée. Récap + QR.</div>`;
  ...
  function clearTable(){
    tbody.innerHTML=''; rowTotal.classList.add('hidden'); totalTime.textContent='—'; totalSpeed.textContent='—';
  ...
  function start(){
    if(state.running){
      ...
      if(!['minuteurSimple','simple','simpleDistance'].includes(state.mode)) btnLap.disabled=true;
      btnStop.disabled=false; btnReset.disabled=false;
      return;
    }
    state.running=true; state.startTime=now(); state.lastLapAt=state.lastLapAt||state.startTime;
    btnStart.textContent='Pause'; btnLap.disabled=(['minuteurSimple','simple','simpleDistance'].includes(state.mode));
    btnStop.disabled=false; btnReset.disabled=false;
  ...
  function addLapRow(cumMs,lapMs){
    ...
    tbody.appendChild(tr); state.laps.push({idx,cumMs,lapMs,lapDist,lapSpeed,cumMeters:cumMetersNow});
  }
  ...
  function updateRecapTotals(){
    ...
    totalTime.textContent=fmt(totalMs); totalSpeed.textContent=(vAvg? vAvg.toFixed(2):'—');
    if(totalDistanceCell) totalDistanceCell.textContent=(totalMeters? Math.round(totalMeters):'—');
    recap.classList.add('hidden'); recapBody.innerHTML='';
    const rows=[];
    ...
    if(rows.length){ recap.classList.remove('hidden'); recapBody.innerHTML=rows.map(([k,v])=>`<tr><td><strong>${k}</strong></td><td>${v}</td></tr>`).join(''); }
  }

  function finish(){
    if(state.running){ state.elapsed=state.elapsed + (now()-state.startTime); state.running=false; cancelAnimationFrame(state.raf); }
    btnLap.disabled=true; btnStart.textContent='Démarrer'; btnStop.disabled=true; results.classList.remove('hidden'); rowTotal.classList.remove('hidden');
    if(['tours','demiCooper','cooper'].includes(state.mode)){
  ...
  function undoFraction(){
    if(!['tours','demiCooper','cooper'].includes(state.mode)) return;
    state.fractionAdded=0; liveDistVal.textContent=Math.round(state.cumDist);
    updateRecapTotals(); generateQR();
  }
  ...
  function buildPayload(){
    const id=identity();
    const base = { version:"1.0", app:"RunMeasure", date_iso:new Date().toISOString(), nom:id.nom, prenom:id.prenom, classe:id.classe, sexe:id.sexe };

    if(state.mode==='intervalles'){
      const extras = { test:"Tps intermédiaires", distance_cible_m: Math.round(state.targetDist||0), pas_intermediaire_m: Math.round(state.splitDist||0) };
      const totalLaps = Math.min(state.laps.length, Math.floor((state.targetDist||0)/(state.splitDist||1)));
      for (let i=0;i<totalLaps;i++){ const dist = (i+1)*(state.splitDist||0); extras[`tps_${dist}`] = fmtHMS(state.laps[i].cumMs); }
      if (totalLaps>0){ const lastCum = state.laps[totalLaps-1].cumMs; extras.temps_total_s = Math.round(lastCum/1000); extras.temps_total_hms = fmtHMS(lastCum); }
      return Object.assign({}, base, extras);
    }

    if(state.mode==='simpleDistance'){
      const d = Math.round(state.targetDist||0); const v = (state.elapsed>0) ? (d/1000)/(state.elapsed/3600000) : 0;
      return Object.assign({}, base, { test:"Chrono + vitesse", distance_m:d, temps_total_s:Math.round(state.elapsed/1000), temps_total_hms:fmtHMS(state.elapsed), vitesse_kmh:Math.round(v*10)/10 });
    }

    if(state.mode==='tours'){
      const dist = Math.round(state.cumDist + state.fractionAdded);
      const v = (state.elapsed>0) ? (dist/1000)/(state.elapsed/3600000) : 0;
      return Object.assign({}, base, { test:"Minuteur + distance", duree_minuteur_s:Math.round(state.elapsed/1000), duree_minuteur_hms:fmtHMS(state.elapsed), distance_realisee_m:dist, vitesse_kmh:Math.round(v*10)/10 });
    }
    ...
    if(state.mode==='demiCooper' || state.mode==='cooper'){
      ...
      return Object.assign({}, base, { test:(state.mode==='demiCooper')?"Demi-Cooper 6min":"Cooper 12min", duree_s:fixed, duree_hms:fmtHMS(fixed*1000), distance_realisee_m:dist, vitesse_moy_kmh:Math.round(v*10)/10, vma_estimee_kmh:Math.round(v*10)/10 });
    }
    return null;
  }
  ...
  function generateQR(){
    ...
    qrcodeBox.innerHTML='';
    if(!p) return;
    try{ new QRCode(qrcodeBox,{ text:JSON.stringify(p), width:220, height:220, correctLevel:QRCode.CorrectLevel.M }); }
  ...
  btnReset.addEventListener('click', ()=>{
    ...
    display.textContent='00:00.0'; display.classList.remove('display-huge');
    if(circleWrap && !circleWrap.classList.contains('hidden')){ if(state.ringTotal) updateRing(state.ringTotal, state.ringTotal);}
    btnStart.textContent='Démarrer'; btnLap.disabled=(['minuteurSimple','simple','simpleDistance'].includes(state.mode)); btnStop.disabled=true;
    if(!['simple','simpleDistance','minuteurSimple'].includes(state.mode)) tableWrap.classList.add('hidden');
    results.classList.add('hidden'); qrcodeBox.innerHTML=''; liveDistVal.textContent='0'; recap.classList.add('hidden'); recapBody.innerHTML=''; if(totalDistanceCell) totalDistanceCell.textContent='—'; clearTable();
  });
