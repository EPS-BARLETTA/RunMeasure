...
(function(){
  document.addEventListener('DOMContentLoaded', ()=>{
    ...
    const TARGET=6*60; let running=false, t0=0, raf=0, laps=0, liveExtra=0, endFrac=0, splits=[], last=0;
    ...
    const finish = ()=>{
      ...
      saveRowToCsv({nom:payload.nom, prenom:payload.prenom, classe:payload.classe, mode:payload.mode, duree_s:payload.duree_s, distance_m:payload.distance_m, fraction_added_m:payload.fraction_added_m||0, splits_ms:(payload.splits_ms||[]).join('|')}, 'mc_6min');
    };
    ...
    on(start,'click',()=>{
      ...
      t0=performance.now(); display.textContent='06:00'; toggleLast10(display,false); setRunning(true); tick(); recalc();
    });
    on(stop,'click',finish);
    on(reset,'click',()=>{ cancelAnimationFrame(raf); setRunning(false); display.textContent='06:00'; display.classList.remove('blink'); qr.innerHTML=''; laps=0; liveExtra=0; endFrac=0; manualExtra.value='0'; splits=[]; splitsBody.innerHTML=''; recalc(); });
