// RunMeasure — Modes.js (QR ScanProf compliant)
// Génère des QR JSON *plats* : nom, prenom, classe, sexe, test + infos du test
// Temps sans millisecondes (HH:MM:SS ou MM:SS).

(() => {
  const $ = (s, root=document) => root.querySelector(s);
  const fmt = (ms) => { // conserve pour l'affichage à l'écran (dixièmes)
  ...
  const fmtHMS_noMs = (ms) => { // pour QR/CSV (sans ms)
  ...
  function clearTable(){ if(!tbody) return; tbody.innerHTML=''; rowTotal?.classList.add('hidden'); if(totalTime) totalTime.textContent='—'; if(totalSpeed) totalSpeed.textContent='—'; if(totalDistanceCell) totalDistanceCell.textContent='—'; }
  ...
  function buildPayload(){
    ...
    if(state.mode==='intervalles'){
      ...
      for (let i=0;i<totalLaps;i++){
        ...
        const key = `tps_${dist}`;
        extras[key] = fmtHMS_noMs(state.laps[i].cumMs);
      }
      if (totalLaps>0){
        const lastCum = state.laps[totalLaps-1].cumMs;
        extras.temps_total_s = Math.round(lastCum/1000);
        extras.temps_total_hms = fmtHMS_noMs(lastCum);
      }
    ...
    if(state.mode==='simpleDistance'){
      ...
      const t = Math.max(0, Math.round(state.elapsed/1000));
      const v = (state.elapsed>0) ? (d/1000)/(state.elapsed/3600000) : 0;
      return Object.assign({}, base, {
        test: "Chrono + vitesse",
        distance_m: d,
        temps_total_s: t,
        temps_total_hms: fmtHMS_noMs(state.elapsed),
    ...
    if(state.mode==='tours'){
      ...
      const t = Math.max(0, Math.round(state.elapsed/1000));
      const v = (state.elapsed>0) ? (dist/1000)/(state.elapsed/3600000) : 0;
      return Object.assign({}, base, {
        test: "Minuteur + distance",
        duree_minuteur_s: t,
        duree_minuteur_hms: fmtHMS_noMs(state.elapsed),
    ...
    if(state.mode==='demiCooper' || state.mode==='cooper'){
      const fixed = (state.mode==='demiCooper') ? 6*60 : 12*60; // secondes
      const dist = Math.round(state.cumDist + state.fractionAdded);
      const v = (dist/1000) / (fixed/3600); // km/h
      return Object.assign({}, base, {
        test: (state.mode==='demiCooper') ? "Demi-Cooper 6min" : "Cooper 12min",
        duree_s: fixed,
        duree_hms: fmtHMS_noMs(fixed*1000),
        distance_realisee_m: dist,
  ...
  function generateQR(){
    ...
    if(!p || !qrcodeBox) return;
    qrcodeBox.innerHTML='';
    try{
      new QRCode(qrcodeBox,{ text:JSON.stringify(p), width:220, height:220, correctLevel:QRCode.CorrectLevel.M });
    }catch(e){
  ...
  function csvExport(){
    ...
    if(state.mode==='intervalles'){
      ...
      state.laps.forEach((l,i)=>{
        ...
        lines.push([s.nom||'',s.prenom||'',s.classe||'',s.sexe||'', Math.round(state.targetDist||0), Math.round(state.splitDist||0), i+1, dist, fmtHMS_noMs(l.cumMs), fmtHMS_noMs(l.lapMs)]);
      });
    } else if(state.mode==='simpleDistance'){
      const head=['nom','prenom','classe','sexe','distance_m','temps_hms','temps_s','vitesse_kmh'];
      const row=[s.nom||'',s.prenom||'',s.classe||'',s.sexe||'', Math.round(state.targetDist||0), fmtHMS_noMs(state.elapsed), Math.round(state.elapsed/1000), ((kmh(state.targetDist,state.elapsed))||0).toFixed(2)];
      ...
      state.laps.forEach(l=>{
        ...
        lines.push([s.nom||'',s.prenom||'',s.classe||'',s.sexe||'', l.idx, fmtHMS_noMs(l.cumMs), fmtHMS_noMs(l.lapMs), v, (l.cumMeters||'')]);
      });
      lines.push([]);
      ...
      const rowRecap=[s.nom||'',s.prenom||'',s.classe||'',s.sexe||'', totalMeters, fmtHMS_noMs(state.elapsed), Math.round(state.elapsed/1000), vavg];
...
');
  ...
  btnReset?.addEventListener('click', ()=>{
    ...
    if(btnStart) btnStart.textContent='Démarrer'; if(btnLap) btnLap.disabled=(['minuteurSimple','simple','simpleDistance'].includes(state.mode)); if(btnStop) btnStop.disabled=true; if(!['simple','simpleDistance','minuteurSimple'].includes(state.mode)) tableWrap?.classList.add('hidden');
    results?.classList.add('hidden'); if(qrcodeBox) qrcodeBox.innerHTML=''; if(liveDistVal) liveDistVal.textContent='0'; recap?.classList.add('hidden'); if(recapBody) recapBody.innerHTML=''; if(totalDistanceCell) totalDistanceCell.textContent='—'; clearTable();
  });
