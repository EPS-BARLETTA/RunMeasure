// RunMeasure V7.6 — interval QR auto, CSV tables per mode, big displays
(() => {
  ...
  const ivlExportBox=$('#intervalles-export'); const circleWrap=$('#circle-wrap'); const liveDistance=$('#live-distance'); const liveDistVal=$('#live-dist-val'); const fractionTools=$('#fraction-tools'); const recap=$('#recap'); const recapBody=$('#recap-body');
    ...
    switch(state.mode){
      case 'intervalles':
        ...
        params.innerHTML = `
          ...
          <div class="info">Appuie sur « Tour ». À la fin : QR auto (par défaut <em>cumulés</em>) ou <em>par intervalle</em> selon la sélection ci-dessous.</div>`;
  ...
  function generateQR(){ const p=buildPayload(); qrcodeBox.innerHTML=''; if(!p) return; try{ new QRCode(qrcodeBox,{ text:JSON.stringify(p), width:220, height:220, correctLevel:QRCode.CorrectLevel.M }); }catch(e){ const d=document.createElement('div'); d.textContent='Erreur QR: '+e.message; qrcodeBox.appendChild(d);} }
