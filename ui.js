...
window.debounce = (fn,w=200)=>{let t=0;return(...a)=>{clearTimeout(t);t=setTimeout(()=>fn(...a),w);}};
...
(function(){
  const key='multichrono_theme'; const saved=localStorage.getItem(key);
  const prefers=window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches;
  const initial = saved ? saved==='dark' : prefers;
  document.documentElement.classList.toggle('dark', initial);
  document.addEventListener('DOMContentLoaded', ()=>{
    const t=qs('#darkToggle'); if(t){ t.checked=initial; on(t,'change',e=>{
